 ...
 * V should be declared unsigned int or perhaps INT32_JPEG.
 */
...
/* Process a DHT marker */
{
  ...
  while(length > 16)
  {
     ...
     */
    if(count > 256 || ((INT32_JPEG) count) > length)
    {
...
examine_app0(j_decompress_ptr cinfo, JOCTET* data,
             unsigned int datalen, INT32_JPEG remaining)
/* Examine first few bytes from an APP0.
 * Take appropriate action if it is a JFIF marker.
 * datalen is # of bytes at data[], remaining is length of rest of marker data.
 */
{
  INT32_JPEG totallen = (INT32_JPEG) datalen + remaining;
      ...
      GETJOCTET(data[4]) == 0)
  {
    ...
    if(totallen !=
        ((INT32_JPEG)GETJOCTET(data[12]) * (INT32_JPEG)GETJOCTET(data[13]) * (INT32_JPEG) 3))
    {
                  ...
                  unsigned int length_limit)
{
   ...
   */
  const long maxlength = cinfo->mem->max_alloc_chunk - (long) SIZEOF(struct jpeg_marker_struct);
